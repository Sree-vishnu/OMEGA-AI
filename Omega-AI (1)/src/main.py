...
from actions import fisat
from actions import conauto
import paho.mqtt.client as mqtt


def on_connect(client, userdata, flags, rc):
  print("Connected with result code "+str(rc))
  client.subscribe([("detect", 0), ("recognize", 0)])

def on_message(client, userdata, msg):


   if msg.topic == "detect":
      sts = msg.payload.decode()
      sts = int(sts)
      print(sts)
      conauto(sts)
      print(".......")
   elif msg.topic == "recognize":
...
def main():
    ...
    with Assistant(credentials, args.device_model_id) as assistant:
        ...
        for event in events:
            ...
            if 'college'.lower() in str(usrcmd).lower():
                assistant.stop_conversation()
                fisat(str(usrcmd).lower())
